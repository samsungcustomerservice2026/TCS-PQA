...
export function buildGoGoKnowledgeContext() {
  ...
  return [
    ...
    '### Never do',
    '- Invent passwords, API keys, Firebase secrets',
...
export function buildGoGoSystemPrompt({ lang = 'en', visitorName = '', learningHint = '' } = {}) {
  ...
  return [
    ...
    `Visitor name: ${name}.`,
    `Reply language: ${L === 'ar' ? 'Modern Standard Arabic (اللغة العربية الفصحى) — NOT Egyptian colloquial, NOT Gulf/Levantine.' : 'English'}.`,
    L === 'ar' ? `Arabic language rules (mandatory):\n${GOGO_AR_VOICE_RULES}` : '',
    L === 'ar'
    ...
    'Keep answers short and clear (2–5 short sentences). In Arabic use فصحى — polite and professional, not slang.',
    ...
    'Arabic generative rule (mandatory): when explaining TCS, name all three lines in one clear Fusha sentence, e.g. "يتابع تي سي اس أداء المهندسين على ثلاثة أقسام: الموبايل، والأجهزة المنزلية، والشاشات."',
    ...
    'Technical Consultants library: answer only from published tip title/summary/extracted text (never invent). Prefer My Knowledge guidance + how to finish a tip. Generic “consultant” questions → guide to My Knowledge / how_tip chips.',
