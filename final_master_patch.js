...
// 1. Define the components separately to ensure clean syntax
...
const HeaderDef = `const Header = ({ onHome, onLogoClick, appMode }) => {
  const showLogo = appMode !== null; 
  const appLogo = useMemo(() => {
    if (appMode === 'PQA_MX') return './pqa_logo.png';
    if (appMode === 'PQA_CE') return './pqa_logo.png';
...
// Replace Header and insert getMxPartner if missing
if (!content.includes('const getMxPartner')) {
    content = content.replace('const getMonthIndex', getMxPartnerDef + '\n\nconst getMonthIndex');
}
const hStart = content.indexOf('const Header =');
const hEnd = content.indexOf('const MetricBar =');
if (hStart !== -1 && hEnd !== -1) {
    content = content.substring(0, hStart) + HeaderDef + '\n\n' + content.substring(hEnd);
}

// Ensure state is present
if (!content.includes('const [pqaMxGroupBy')) {
...
// Correcting the PQA_DIVISION_SELECTION and HOME view blocks
const divSelOldBlock = content.indexOf("{view === 'PQA_DIVISION_SELECTION' && (");
const homeViewOldBlock = content.indexOf("{view === 'HOME' && (");
const engLookupOldBlock = content.indexOf("{view === 'ENGINEER_LOOKUP' && (");

if (divSelOldBlock !== -1 && homeViewOldBlock !== -1) {
    const newDivSel = `{view === 'PQA_DIVISION_SELECTION' && (
    ...
    content = content.substring(0, divSelOldBlock) + newDivSel + '\\n\\n          ' + content.substring(homeViewOldBlock);
}

// Re-apply the Dashboard Toggle in the HOME view
const newDashboardToggle = `{/* Dashboard Toggle */}
...
const tsStart = content.indexOf('{/* Dashboard Toggle */}');
if (tsStart !== -1) {
    const tsMid = content.indexOf('Monthly', tsStart);
    const tsEnd = content.indexOf('</div>', tsMid);
    // Find the end of the outer div
    const outerEnd = content.indexOf('</div>', tsEnd + 6);
    content = content.substring(0, tsStart) + newDashboardToggle + content.substring(outerEnd + 6);
}

fs.writeFileSync(path, content);
console.log('Final Master Patch executed successfully');
