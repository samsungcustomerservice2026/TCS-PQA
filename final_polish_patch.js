...
// 1. Header Fix: Hide logo on homepage, show slogan in center
...
// 2. Dashboard Toggles Fix: Add By Partner / By Center
...
const toggleStart = "{/* Dashboard Toggle */}";
// Find the end of that section
const toggleMarker = "<div className=\"flex justify-center\">";
const tsIdx = content.indexOf(toggleStart);
const tmIdx = content.indexOf("<button", tsIdx + toggleStart.length);
const endDivIdx = content.indexOf("</div>", tmIdx); // end of Monthly/Quarterly row
const outerDivIdx = content.indexOf("</div>", endDivIdx + 6); // end of outer wrapper

if (tsIdx !== -1) {
    content = content.substring(0, tsIdx) + newToggles + content.substring(outerDivIdx + 6);
}

// 3. Select Division: Enlarge MX/CE logos to fit frame
...
const divSelStart = "{view === 'PQA_DIVISION_SELECTION' && (";
const dIdx = content.indexOf(divSelStart);
if (dIdx !== -1) {
    const endBlock = ")}"; 
    const deIdx = content.indexOf(endBlock, dIdx);
    content = content.substring(0, dIdx) + newDivisionSelection + content.substring(deIdx + 2);
}

fs.writeFileSync(path, content);
console.log('Final Polish Patch complete');
